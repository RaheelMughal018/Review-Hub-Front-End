...
import { PieChart, Pie, Tooltip, Legend, BarChart, XAxis, YAxis, Bar } from 'recharts';
...
const ChartComponent = () => {
  ...
  const [text, setText] = useState('Hello Their you Sexy Sexy Sexy sexy Zubair');
  ...
  const sendRecommendationData = async (recommendationData) => {
    try {
      ...
      const response = await fetch('http://127.0.0.1:8000/recommendation_summary/', {
        method: 'POST',
        headers: {
  ...
  useEffect(() => {
    ...
    const recommendationData = parsedData.filter(item => item.annotation === 'Recommendation');
  ...
  const prepareWordCloudData = () => {
    ...
    data.forEach(item => {
      ...
      words.forEach(word => {
        ...
        if (!stopWords.includes(cleanedWord)) { // Check if word is not in stopWords array
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
      ...
      <StarsCanvas />
      {/* Word Cloud */}
      <h1 className={`${styles.heroHeadText} text-white`} style={{ marginBottom: '20px', marginTop: '70px' }}>Word Cloud</h1>
      <div style={{ width: '1000%', maxWidth: '1500px', height: '400px' }}> {/* Adjust width as needed */}
        <WordCloud
      ...
      <h1 className={`${styles.heroHeadText} text-white`}
        style={{ margin: '30px' }}>Summary by Rev</h1>
      <button
      ...
      <div style={{ marginBottom: '20px', width: '100%', maxWidth: '1200px', margin: '0 auto', textAlign: 'center' }}>
        {/* Input Field */}
        <textarea
          value={text}
      ...
      <Slideshow label="Recommendation" data={data} style={{ margin: '20px' }} />
